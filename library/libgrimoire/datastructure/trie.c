...
const int radix_default = 256;
const uint8_t mapping_table_default[256] = {
...
trie_layer_t * trie_find(trie_t * this, void * src, int src_len)
{
	priv_trie_t * priv = (priv_trie_t *)this;
	uint8_t * byte = src;
	trie_layer_t * layer;
	...
	for(i=0;i<src_len;i++)
	{
		layer = layer->get_next(layer, priv->mapping_table[byte[i]]);
		if(layer == NULL)
			break;
	}
